...
<div id="batchZoomSessionsModal" class="admin-modal">
    <div class="admin-modal-content" style="max-width:920px">
        <div class="admin-modal-header"><div><h3 class="admin-modal-title">Batch Zoom Calendar</h3><p id="zoom_batch_title" class="muted" style="margin:.25rem 0 0"></p></div><button type="button" class="admin-modal-close" onclick="closeModal('batchZoomSessionsModal')">&times;</button></div>
        <div class="admin-modal-body">
            <form id="batchZoomSessionForm" class="panel" style="padding:1rem;margin-bottom:1rem">
                <input type="hidden" id="zoom_session_id">
                <div class="form-grid" style="margin:0">
                    <div class="field"><label>Session title</label><input id="zoom_session_title" required placeholder="Cardiovascular Disorders Live Review"></div>
                    <div class="field"><label>Zoom meeting link</label><input id="zoom_session_url" type="url" required placeholder="https://zoom.us/j/..."></div>
                    <div class="field"><label>Starts</label><input id="zoom_session_starts_at" type="datetime-local" required></div>
                    <div class="field"><label>Ends</label><input id="zoom_session_ends_at" type="datetime-local"></div>
                    <div class="field"><label>Status</label><select id="zoom_session_status"><option value="scheduled">Scheduled</option><option value="cancelled">Cancelled</option></select></div>
                    <div class="field full"><label>Description</label><textarea id="zoom_session_description" rows="2" placeholder="What will be covered during this live session"></textarea></div>
                </div>
                <div style="display:flex;justify-content:flex-end;gap:.5rem;margin-top:1rem"><button type="button" class="btn-ghost" onclick="resetZoomSessionForm()">Clear</button><button type="submit" class="btn-primary">Save Zoom Session</button></div>
            </form>
            <div class="table-wrap"><table class="data-table"><thead><tr><th>Session</th><th>Date and time</th><th>Status</th><th>Action</th></tr></thead><tbody id="batch_zoom_sessions_body"></tbody></table></div>
        </div>
    </div>
</div>

<div id="deleteZoomSessionModal" class="admin-modal" style="z-index:10030">
    <div class="admin-modal-content" style="max-width:430px;text-align:center">
        <div class="admin-modal-header" style="justify-content:center;border-bottom:0"><h3 class="admin-modal-title">Delete Zoom Session?</h3></div>
        <div class="admin-modal-body"><p style="margin:0;color:var(--text-muted)">This removes the scheduled session from this batch’s learner calendar.</p></div>
        <div class="admin-modal-footer" style="justify-content:center;border-top:0"><button type="button" class="btn-ghost" onclick="closeModal('deleteZoomSessionModal')">Cancel</button><button type="button" id="confirm_delete_zoom_session" class="btn-primary" style="background:var(--wrong)">Delete Session</button></div>
    </div>
</div>
...
<script>
    ...
    let zoomBatchId = null;
    let loadedZoomSessions = [];
    let pendingZoomSessionDeleteId = null;
    ...
    function resetZoomSessionForm(){['zoom_session_id','zoom_session_title','zoom_session_url','zoom_session_starts_at','zoom_session_ends_at','zoom_session_description'].forEach(id=>document.getElementById(id).value='');document.getElementById('zoom_session_status').value='scheduled';}
    async function openBatchZoomSessions(courseId,batchId){batchCourseId=courseId;zoomBatchId=batchId;resetZoomSessionForm();closeModal('courseBatchesModal');openModal('batchZoomSessionsModal');await loadBatchZoomSessions();}
    function editZoomSession(id){const session=loadedZoomSessions.find(item=>item.id===Number(id));if(!session)return;document.getElementById('zoom_session_id').value=session.id;document.getElementById('zoom_session_title').value=session.title||'';document.getElementById('zoom_session_url').value=session.zoomUrl||'';document.getElementById('zoom_session_starts_at').value=(session.startsAt||'').slice(0,16);document.getElementById('zoom_session_ends_at').value=(session.endsAt||'').slice(0,16);document.getElementById('zoom_session_description').value=session.description||'';document.getElementById('zoom_session_status').value=session.status||'scheduled';}
    async function loadBatchZoomSessions(){const body=document.getElementById('batch_zoom_sessions_body');body.innerHTML='<tr><td colspan="4" class="muted">Loading Zoom calendar…</td></tr>';const response=await fetch(`/admin/content/courses/${batchCourseId}/batches/${zoomBatchId}/zoom-sessions`,{headers:{'Accept':'application/json','X-Requested-With':'XMLHttpRequest'},credentials:'same-origin'});const data=await response.json();if(!response.ok||!data.success){body.innerHTML=`<tr><td colspan="4">${escapeEnrollmentText(data.message||'Unable to load Zoom calendar.')}</td></tr>`;return;}loadedZoomSessions=data.sessions||[];document.getElementById('zoom_batch_title').textContent=`${data.batch.name} · ${data.batch.code}`;body.innerHTML=loadedZoomSessions.length?loadedZoomSessions.map(item=>`<tr><td><strong>${escapeEnrollmentText(item.title)}</strong><br><small>${escapeEnrollmentText(item.description||'')}</small></td><td>${new Date(item.startsAt).toLocaleString()}${item.endsAt?`<br><small>until ${new Date(item.endsAt).toLocaleString()}</small>`:''}</td><td><span class="status ${item.status==='scheduled'?'success':'warning'}">${escapeEnrollmentText(item.status)}</span></td><td><div style="display:flex;gap:.35rem"><button type="button" class="btn-ghost" onclick="editZoomSession(${item.id})">Edit</button><button type="button" class="btn-ghost" style="color:var(--wrong)" onclick="requestDeleteZoomSession(${item.id})">Delete</button></div></td></tr>`).join(''):'<tr><td colspan="4" class="muted">No Zoom sessions scheduled for this batch.</td></tr>';}
    document.getElementById('batchZoomSessionForm').addEventListener('submit',async event=>{event.preventDefault();const id=document.getElementById('zoom_session_id').value;const payload={title:document.getElementById('zoom_session_title').value,zoom_url:document.getElementById('zoom_session_url').value,starts_at:document.getElementById('zoom_session_starts_at').value,ends_at:document.getElementById('zoom_session_ends_at').value||null,description:document.getElementById('zoom_session_description').value||null,status:document.getElementById('zoom_session_status').value};const response=await fetch(`/admin/content/courses/${batchCourseId}/batches/${zoomBatchId}/zoom-sessions${id?`/${id}`:''}`,{method:id?'PUT':'POST',headers:{'Accept':'application/json','Content-Type':'application/json','X-CSRF-TOKEN':@json(csrf_token()),'X-Requested-With':'XMLHttpRequest'},credentials:'same-origin',body:JSON.stringify(payload)});const data=await response.json();if(!response.ok||!data.success)return showAdminAlert(Object.values(data.errors||{}).flat().join(' ')||data.message||'Unable to save Zoom session.');resetZoomSessionForm();await loadBatchZoomSessions();});
    async function deleteZoomSession(id){const response=await fetch(`/admin/content/courses/${batchCourseId}/batches/${zoomBatchId}/zoom-sessions/${id}`,{method:'DELETE',headers:{'Accept':'application/json','X-CSRF-TOKEN':@json(csrf_token()),'X-Requested-With':'XMLHttpRequest'},credentials:'same-origin'});const data=await response.json();if(!response.ok||!data.success)return showAdminAlert(data.message||'Unable to delete Zoom session.');await loadBatchZoomSessions();}
    function requestDeleteZoomSession(id){pendingZoomSessionDeleteId=id;openModal('deleteZoomSessionModal');}
    document.getElementById('confirm_delete_zoom_session').addEventListener('click',async()=>{if(!pendingZoomSessionDeleteId)return;const id=pendingZoomSessionDeleteId;pendingZoomSessionDeleteId=null;closeModal('deleteZoomSessionModal');await deleteZoomSession(id);});
